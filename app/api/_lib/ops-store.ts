...
export async function ensureOpsSchema() {
  ...
  if (!schemaReady) {
    schemaReady = (async () => {
      // Most requests run on an already-migrated database. A cheap catalog probe avoids
      // replaying every CREATE TABLE / CREATE INDEX statement on each serverless cold start.
