...
export async function POST(request:Request){
  try{
    ...
    if(action==='add'){
      ...
      await sql`INSERT INTO kitchen_prep_items (prep_date,section,item,quantity,notes,sort_order,created_at,created_by) VALUES (${prepDate},${section},${item},${clean(body.quantity,80)},${clean(body.notes,600)},${Number(body.sortOrder)||0},${now},${clean(body.createdBy,120)})`;
    } else if(action==='toggle'){
      const id=Number(body.id); if(!id)return Response.json({error:'id required'},{status:400});
      const completed=!!body.completed;
      await sql`UPDATE kitchen_prep_items SET completed=${completed},completed_at=${completed?now:null},completed_by=${completed?clean(body.completedBy,120):''} WHERE id=${id}`;
